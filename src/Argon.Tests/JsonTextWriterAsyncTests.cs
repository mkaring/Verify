  ...
  ""prop"": true
    ...
    public async Task NullableValueFormattingAsync()
    {
        ...
        using (var jsonWriter = new JsonTextWriter(stringWriter))
        {
            ...
            await jsonWriter.WriteValueAsync((DateTime?)new DateTime(TestFixtureBase.InitialJavaScriptDateTicks, DateTimeKind.Utc));
            await jsonWriter.WriteValueAsync((DateTimeOffset?)null);
            await jsonWriter.WriteValueAsync((DateTimeOffset?)new DateTimeOffset(TestFixtureBase.InitialJavaScriptDateTicks, TimeSpan.Zero));
            await jsonWriter.WriteEndArrayAsync();
        }
